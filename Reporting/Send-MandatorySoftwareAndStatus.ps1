...
$buildOutputPaths =  Get-ChildItem ([string]::Concat($settings.OutputFolder,"\*\","mandatorySoftware.",$settings.OutputFileType))
$data = @()
...
$buildOutputPaths |  ForEach-Object {
    if($settings.OutputFileType -eq "csv"){
        $temp = Import-Csv   $_.FullName
          ...
          $data += ($temp | Where-Object {$_.installed -ne $true -and $_.Service_Running -ne $true})
        }
}
$data
